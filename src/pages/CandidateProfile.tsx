...
export default function CandidateProfilePage() {
  ...
  const fetchCandidateProfile = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/users/${candidateId}`, { headers: authHeaders() });
      ...
      const suggestedRes = await fetch(`/api/candidates/suggested?limit=5&excludeId=${candidateId}`, { headers: authHeaders() });
      if (suggestedRes.ok) {
