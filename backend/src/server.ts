...
app.use(cors({ origin: (origin, cb) => {
  if (!origin) return cb(null, true); // allow non-browser requests like curl/postman
...
// Middleware types and auth helpers
...
const userAuth = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const auth = req.headers.authorization;
  if (!auth) return res.status(401).json({ success: false, message: 'Token manquant' });
  ...
  try {
    ...
    console.error('userAuth verify error:', (err as Error).stack || (err as Error).message || err);
...
const adminAuth = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    (req as any).userId = decoded.id; // admin id
    (req as any).userRole = decoded.role;
    next();
  } catch (err) {
    return res.status(401).json({ success: false, message: 'Token invalide' });
...
).catch((err) => console.error("Could not add application_url column:", err));
...
app.post("/api/jobs", userAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (req.userRole && String(req.userRole).toLowerCase() === 'company') {
      ...
      if (urows.length > 0) companyName = urows[0].company_name;
    }
    ...
    const { rows } = await pool.query(
      `INSERT INTO jobs (title, company, company_id, location, sector, type, salary, description, application_url, application_via_emploi, deadline, is_company_owned, published)
    ...
    res.status(500).json({ success: false, message: (err as Error).message || "Erreur serveur" });
  }
});
...
app.get('/api/jobs/:id', async (req, res) => {
  try {
    ...
    if (!rows || rows.length === 0) return res.status(404).json({ success: false, message: 'Offre introuvable' });
    const job = rows[0];
...
app.put("/api/jobs/:id", userAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    const { title, company: bodyCompany, location, sector = null, type, salary, description, application_url, application_via_emploi = false, deadline = null } = req.body;
    ...
    if (req.userRole && String(req.userRole).toLowerCase() === 'company') {
      ...
      if (!jrows || jrows.length === 0) return res.status(404).json({ success: false, message: 'Offre introuvable' });
    ...
    if (bodyCompany) {
      await pool.query(
        `UPDATE jobs SET title=$1, company=$2, location=$3, sector=$4, type=$5, salary=$6, description=$7, application_url=$8, application_via_emploi=$9, deadline=$10 WHERE id=$11`,
        [title, bodyCompany, location, sector, type, salary || null, description, application_url || null, application_via_emploi, deadline || null, id]
      );
    } else {
      await pool.query(
        `UPDATE jobs SET title=$1, location=$2, sector=$3, type=$4, salary=$5, description=$6, application_url=$7, application_via_emploi=$8, deadline=$9 WHERE id=$10`,
        [title, location, sector, type, salary || null, description, application_url || null, application_via_emploi, deadline || null, id]
      );
    }
...
pool.query(
  `UPDATE jobs SET is_company_owned = true
   FROM users u
   WHERE jobs.company_id = u.id
     AND lower(coalesce(u.user_type,'')) = 'company'
     AND jobs.created_at >= u.created_at`
).then((r:any) => console.log('Backfilled jobs.is_company_owned rows:', r.rowCount)).catch((err:any) => console.error('Error backfilling is_company_owned', err));
...
pool.query(
  `UPDATE jobs SET company_id = u.id
   FROM users u
   WHERE jobs.company_id IS NULL
     AND lower(trim(coalesce(jobs.company,''))) = lower(trim(coalesce(u.company_name,'')))
     AND lower(coalesce(u.user_type,'')) = 'company'`
).then((r:any) => console.log('Backfilled jobs.company_id rows:', r.rowCount)).catch((err:any) => console.error('Error backfilling jobs.company_id', err));

// Backfill company_id on job_applications from jobs table when missing
pool.query(
  `UPDATE job_applications ja SET company_id = j.company_id
   FROM jobs j
   WHERE ja.company_id IS NULL
     AND ja.job_id = j.id
     AND j.company_id IS NOT NULL`
).then((r:any) => console.log('Backfilled job_applications.company_id rows:', r.rowCount)).catch((err:any) => console.error('Error backfilling job_applications.company_id', err));

// Further backfill: match company name from jobs to users table for applications still missing company_id
pool.query(
  `UPDATE job_applications ja SET company_id = u.id
   FROM jobs j, users u
   WHERE ja.company_id IS NULL
     AND ja.job_id = j.id
     AND j.company IS NOT NULL
     AND lower(trim(u.company_name)) = lower(trim(j.company))`
).then((r:any) => console.log('Backfilled job_applications.company_id by company name:', r.rowCount)).catch((err:any) => console.error('Error backfilling by company name', err));


// (auth middleware defined earlier)

// Submit a job application (candidate)
app.post('/api/job-applications', userAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const applicantId = req.userId;
    const { job_id, cv_url = null, cover_letter_url = null, additional_docs = [], message = null } = req.body;
    if (!applicantId) return res.status(401).json({ success: false, message: 'Utilisateur non authentifié' });
    ...
    if (!jobRows || jobRows.length === 0) return res.status(404).json({ success: false, message: 'Offre introuvable' });
    const job = jobRows[0];
    ...
    const { rows: ins } = await pool.query(
      `INSERT INTO job_applications (job_id, applicant_id, company_id, cv_url, cover_letter_url, additional_docs, message)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [job_id, applicantId, companyId, cv_url, cover_letter_url, JSON.stringify(additional_docs), message]
    );
    ...
    res.status(500).json({ success: false, message: (err as Error).message || 'Erreur' });
  }
});

// Get applications for the authenticated company
app.get('/api/company/applications', userAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (!urows || urows.length === 0) return res.status(404).json({ success: false });
    if (String(urows[0].user_type).toLowerCase() !== 'company') return res.status(403).json({ success: false, message: 'Accès réservé aux entreprises' });
...
app.get('/api/company/validations', userAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.userId;
    if (!userId) return res.status(401).json({ success: false, message: 'Utilisateur non authentifié' });
    const { rows: urows } = await pool.query('SELECT user_type FROM users WHERE id = $1', [userId]);
    if (!urows || urows.length === 0) return res.status(404).json({ success: false });
    if (urows[0].user_type !== 'company') return res.status(403).json({ success: false, message: 'Accès réservé aux entreprises' });
...
app.get('/api/admin/validations', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (status) { params.push(status); where += ` AND ja.status = $${params.length}`; }
    if (companyId) { params.push(companyId); where += ` AND ja.company_id = $${params.length}`; }
...
app.post('/api/company/applications/:id/interview', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!appRows || appRows.length === 0) return res.status(404).json({ success: false, message: 'Candidature introuvable' });
    ...
    if (!owns) return res.status(403).json({ success: false, message: 'Accès refusé' });
    ...
    if (date_time) defaultMessage += `\nDate & heure : ${date_time}`;
    if (place) defaultMessage += `\nLieu : ${place}`;
...
app.post('/api/company/applications/:id/validate', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const companyId = req.userId;
    const { id } = req.params; // application id
    const { date_time, place, message: customMessage } = req.body;
...
app.post('/api/applications/:id/accept', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    const { id } = req.params; // application id
    ...
    if (Number(appRow.applicant_id) !== Number(userId)) return res.status(403).json({ success: false, message: 'Accès refusé' });
...
app.post('/api/applications/:id/decline', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    const { id } = req.params; // application id
    const { message } = req.body;
...
app.get('/api/applications', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    if (!userId) return res.status(401).json({ success: false, message: 'Utilisateur non authentifié' });

    const { rows } = await pool.query(
      `SELECT ja.*, j.title as job_title, j.company as job_company, u.full_name as company_name
       FROM job_applications ja
       LEFT JOIN jobs j ON j.id = ja.job_id
...
app.patch("/api/jobs/:id/publish", userAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    ...
    if (req.userRole && String(req.userRole).toLowerCase() === 'company') {
      ...
      if (!jrows || jrows.length === 0) return res.status(404).json({ success: false });
      const jobCompanyId = jrows[0].company_id;
      if (Number(jobCompanyId) !== Number(req.userId)) return res.status(403).json({ success: false, message: 'Accès refusé' });
    }
...
app.put("/api/formations/:id", async (req, res) => {
  try {
    ...
    const { title, category, level, duration, price, description } = req.body;
    const { rows } = await pool.query(
      `UPDATE formations SET title=$1, category=$2, level=$3, duration=$4, price=$5, description=$6 WHERE id=$7
       RETURNING *`,
      [title, category, level, duration, price || null, description, id]
    );
    res.json({ success: true, formation: rows[0] });
  } catch (err) {
    res.status(500).json({ success: false });
...
app.get('/api/users/me', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    if (!userId) {
    ...
    try {
      ...
      console.error('Primary profile query failed, attempting fallback. Error:', (errQuery as Error).stack || errQuery);
      try {
        ...
        console.error('Fallback profile query also failed:', (err2 as Error).stack || err2);
        throw err2;
      }
    }
  } catch (err) {
    console.error('Get profile error:', (err as Error).stack || (err as Error).message || err);
    res.status(500).json({ success: false, message: (err as Error).message || 'Erreur serveur' });
  }
});
...
app.post("/api/users", async (req, res) => {
  try {
    ...
    if (typeof user_type === 'string') {
      ...
      if (ut === 'candidat' || ut === 'candidate') user_type = 'candidate';
      else if (ut === 'entreprise' || ut === 'company' || ut === 'entrepreneur') user_type = 'company';
      else user_type = 'candidate';
    } else {
      user_type = 'candidate';
    }
    ...
    const { rows } = await pool.query(
      `INSERT INTO users (full_name, email, user_type)
       VALUES ($1, $2, $3)
       RETURNING id, full_name, email, user_type, created_at, is_blocked`,
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    // Basic validations
    const emailRe = /^[^@\s]+@[^@\s]+\.[^@\s]+$/;
    if (!emailRe.test(String(email))) return res.status(400).json({ success: false, message: 'Email invalide' });
    if (String(password).length < 8) return res.status(400).json({ success: false, message: 'Mot de passe trop court (>=8 caractères)' });
    if (phone && !/^[0-9+\-\s()]{6,20}$/.test(String(phone))) return res.status(400).json({ success: false, message: 'Numéro de téléphone invalide' });
    ...
    const hashed = bcrypt.hashSync(password, 10);
    const { rows } = await pool.query(
      `INSERT INTO users (full_name, email, password, user_type, company_name, company_address, phone, country, is_verified)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,false)
       RETURNING id, full_name, email, user_type, company_name, company_address, phone, country, created_at`,
      [full_name || null, email, hashed, user_type, company_name || null, company_address || null, phone || null, country || null]
    );
...
app.post('/api/company-recommendations', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    if (!userId) return res.status(401).json({ success: false, message: 'Utilisateur non authentifié' });
    ...
    await pool.query(
      `INSERT INTO company_recommendations (company_id, skill, diploma, profession, certification) VALUES ($1, $2, $3, $4, $5)`,
      [userId, skill, diploma, profession, certification]
    );
...
app.put('/api/users/me', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    const { 
      full_name, 
    ...
    const { rows } = await pool.query(
      `UPDATE users SET full_name=$1, company_name=$2, company_address=$3, phone=$4, profession=$5, job_title=$6, diploma=$7, experience_years=$8, profile_image_url=$9, skills=$10, country=$11, city=$12, address=$13, birthdate=$14, gender=$15, nationality=$16, qualification=$17, years_experience=$18, documents=$19, contract_type=$20, availability=$21, salary=$22, linkedin=$23, bio=$24, email=$25, updated_at=NOW() WHERE id=$26 RETURNING id, full_name, email, user_type, company_name, company_address, phone, profession, job_title, diploma, experience_years, profile_image_url, skills, country, city, address, birthdate, gender, nationality, qualification, years_experience, documents, contract_type, availability, salary, linkedin, bio, is_verified, created_at, updated_at`,
      [full_name || null, company_name || null, company_address || null, phone || null, profession || null, job_title || null, diploma || null, experience_years || 0, profile_image_url || null, skills || null, country || null, city || null, address || null, birthdate || null, gender || null, nationality || null, qualification || null, years_experience || null, documents || {}, contract_type || null, availability || null, salary || null, linkedin || null, bio || null, email || null, userId]
    );
    res.json(rows[0]);
  } catch (err) {
...
app.put('/api/users/me/password', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
...
app.delete('/api/users/me', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
...
app.get("/api/jobs/recommendations/for-me", userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    ...
    const { rows: userRows } = await pool.query(
      `SELECT profession, skills, preferred_sector, preferred_location, preferred_contract_type 
    ...
    const recommendations = jobs.map((job: Record<string, unknown>) => {
      let score = 0;
      ...
      if (profession && title.includes(profession)) score += 30;
      if (profession && desc.includes(profession)) score += 15;
      
      // Match skills
      for (const skill of skills) {
        const skillLower = String(skill).toLowerCase();
        if (title.includes(skillLower)) score += 20;
        if (desc.includes(skillLower)) score += 15;
      }
      
      // Match sector preference
      if (preferredSector && sector.includes(preferredSector)) score += 25;
      if (preferredSector && desc.includes(preferredSector)) score += 10;
      
      // Match location preference
      if (preferredLocation && location.includes(preferredLocation)) score += 20;
      
      // Match contract type preference
      if (preferredType && type.includes(preferredType)) score += 15;
      
      return { ...job, recommendation_score: score };
    }).filter((j: Record<string, unknown>) => Number(j.recommendation_score) > 0).sort((a: Record<string, unknown>, b: Record<string, unknown>) => Number(b.recommendation_score) - Number(a.recommendation_score)).slice(0, 5);
...
app.put("/api/users/:id/job-preferences", userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    const { id } = req.params;
    ...
    const { rows } = await pool.query(
      `UPDATE users SET preferred_sector = $1, preferred_location = $2, preferred_contract_type = $3 
       WHERE id = $4 
       RETURNING id, preferred_sector, preferred_location, preferred_contract_type`,
      [preferred_sector || null, preferred_location || null, preferred_contract_type || null, id]
    );
...
// ──────────────────────────────────────────────────
app.get('/api/user-documents', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    const { rows } = await pool.query(
      'SELECT id, user_id, doc_type, title, storage_url, created_at FROM user_documents WHERE user_id = $1 ORDER BY created_at DESC',
      [userId]
    );
    res.json(rows);
  } catch (err) {
...
app.post('/api/user-documents', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    ...
    const { rows } = await pool.query(
      'INSERT INTO user_documents (user_id, doc_type, title, storage_url) VALUES ($1, $2, $3, $4) RETURNING *',
      [userId, doc_type, title || null, storage_url]
    );
    ...
    if (doc_type === 'identity') {
      try {
        ...
        if (user && user.is_verified) {
          ...
          await pool.query(
            'INSERT INTO verification_requests (user_id, requested_name, phone, status, reason) VALUES ($1, $2, $3, $4, $5)',
            [userId, user.full_name || null, user.phone || null, 'revoked', 'Identity document changed - certification revoked']
          );
...
app.delete('/api/user-documents/:id', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    const { id } = req.params;
...
// ──────────────────────────────────────────────────
app.get('/api/user-skills', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    const { rows } = await pool.query(
      'SELECT id, user_id, skill_name, category, created_at FROM user_skills WHERE user_id = $1 ORDER BY created_at DESC',
      [userId]
    );
    res.json(rows);
  } catch (err) {
...
app.post('/api/user-skills', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    ...
    const { rows } = await pool.query(
      'INSERT INTO user_skills (user_id, skill_name, category) VALUES ($1, $2, $3) RETURNING *',
      [userId, skill_name, category || 'general']
    );
...
app.delete('/api/user-skills/:id', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    const { id } = req.params;
...
app.get('/api/notifications', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    ...
    const { rows: siteRows } = await pool.query(
      `SELECT id, title, message, target, category, image_url, link, created_at FROM site_notifications WHERE target = $1 OR target = 'all' ORDER BY created_at DESC`,
      [userRole]
    );

    // Personal notifications
    const { rows: personalRows } = await pool.query(
      `SELECT id, title, message, read, created_at FROM notifications WHERE user_id = $1 ORDER BY created_at DESC`,
      [userId]
    );

    // Merge and sort by created_at descending
    const merged = [...(siteRows || []), ...(personalRows || [])].sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
...
const sseClients: Map<number, Set<any>> = new Map();
...
app.get('/api/company/applications/stream', async (req, res) => {
  try {
    ...
    if (!userId || userRole !== 'company') return res.status(403).end();
    ...
    req.on('close', () => {
      try { sseClients.get(id)?.delete(res); } catch (e) {}
    });
  } catch (err) {
...
app.get('/api/company/jobs', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.post('/api/admin/site-notifications', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.delete('/api/admin/site-notifications/:id', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
app.put('/api/admin/site-notifications/:id', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { title, message, target, category, image_url, link } = req.body;
    if (!title) return res.status(400).json({ success: false, message: 'Title required' });
    const { rows } = await pool.query(
      `UPDATE site_notifications SET title = $1, message = $2, target = $3, category = $4, image_url = $5, link = $6, updated_at = NOW() WHERE id = $7 RETURNING *`,
      [title, message || null, target || 'all', category || null, image_url || null, link || null, id]
    );
    res.json({ success: true, notification: rows[0] });
  } catch (err) {
...
app.get('/api/site-notifications', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.get('/api/admin/welcome-templates', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.post('/api/admin/welcome-templates', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const adminId = req.userId;
    const { user_type, title, message } = req.body;
    if (!user_type || !title || !message) return res.status(400).json({ success: false, message: 'Champs requis manquants' });
...
app.put('/api/admin/welcome-templates/:id', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { user_type, title, message } = req.body;
    if (!user_type || !title || !message) return res.status(400).json({ success: false, message: 'Champs requis manquants' });
...
app.delete('/api/admin/welcome-templates/:id', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
app.post('/api/verify-request', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    ...
    if (!user.phone) return res.status(400).json({ success: false, message: 'Numéro de téléphone requis pour la vérification' });
    ...
    if (user.user_type !== 'company') {
      ...
      if (idDocs.length === 0) return res.status(400).json({ success: false, message: 'Pièce d\'identité requise pour la vérification' });
      ...
      if (!user.company_name) return res.status(400).json({ success: false, message: 'Nom de l\'entreprise requis pour la vérification' });
      ...
      if (rccm.length === 0) return res.status(400).json({ success: false, message: 'RCCM de l\'entreprise requis pour la vérification' });
      if (guarantor.length === 0) return res.status(400).json({ success: false, message: 'Pièce d\'identité du garant requise pour la vérification' });
    ...
    const { rows } = await pool.query(
      'INSERT INTO verification_requests (user_id, requested_name, phone, status) VALUES ($1, $2, $3, $4) RETURNING *',
      [userId, requested_name, user.phone || null, 'pending']
    );
...
app.get('/api/admin/verify-requests', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.get('/api/admin/certified-users', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { rows } = await pool.query(
      `SELECT u.id, u.full_name, u.email, u.user_type, u.phone, u.company_name, u.created_at,
        COALESCE(json_agg(json_build_object('doc_type', ud.doc_type, 'storage_url', ud.storage_url)) FILTER (WHERE ud.id IS NOT NULL), '[]') AS documents
...
app.post('/api/notifications/:id/read', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    const { id } = req.params;
...
app.post('/api/notifications/mark-all-read', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
...
app.post('/api/admin/verify-requests/:id/approve', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const adminId = req.userId;
    const { rows } = await pool.query('SELECT * FROM verification_requests WHERE id = $1', [id]);
    if (rows.length === 0) return res.status(404).json({ success: false, message: 'Demande introuvable' });
    const reqRow = rows[0];
...
app.post('/api/admin/verify-requests/:id/reject', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const adminId = req.userId;
...
app.put("/api/users/:id", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE users SET full_name=$1, email=$2, is_blocked=$3 WHERE id=$4
       RETURNING id, full_name, email, user_type, created_at, is_blocked`,
      [full_name, email, is_blocked, id]
    );
    res.json({ success: true, user: rows[0] });
  } catch (err) {
    console.error(err);
...
app.post('/api/users/me/soft-delete', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    // schedule deletion in 14 days
    await pool.query(
      `UPDATE users SET is_blocked = true, deletion_requested_at = NOW(), deletion_scheduled_at = NOW() + interval '14 days', is_deleted = false WHERE id = $1`,
      [userId]
    );
...
app.post('/api/admin/users/:id/delete-now', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    await pool.query('DELETE FROM user_documents WHERE user_id = $1', [id]);
    await pool.query('DELETE FROM user_skills WHERE user_id = $1', [id]);
...
app.post("/api/faqs", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO faqs (question, answer) VALUES ($1, $2) RETURNING id, question, answer, created_at`,
      [question, answer]
    );
...
app.put("/api/faqs/:id", async (req, res) => {
  try {
    ...
    const { question, answer } = req.body;
    const { rows } = await pool.query(
      `UPDATE faqs SET question=$1, answer=$2 WHERE id=$3 RETURNING id, question, answer, created_at`,
      [question, answer, id]
    );
    res.json({ success: true, faq: rows[0] });
  } catch (err) {
...
app.post('/api/publications', adminAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { content, visibility = 'public', hashtags, image_url } = req.body;
    const authorId = req.userId;
    const { rows } = await pool.query(
      `INSERT INTO publications (author_id, content, visibility, hashtags, image_url) VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [authorId || null, content, visibility, hashtags || null, image_url || null]
    );
...
app.put('/api/publications/:id', adminAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    const { content, visibility, hashtags, image_url } = req.body;
    const { rows } = await pool.query(
      `UPDATE publications SET content=$1, visibility=$2, hashtags=$3, image_url=$4 WHERE id=$5 RETURNING *`,
      [content, visibility || 'public', hashtags || null, image_url || null, id]
    );
    res.json({ success: true, publication: rows[0] });
  } catch (err) {
...
app.delete('/api/publications/:id', adminAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
app.post('/api/catalogs', adminAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { service_category, title, description, price = 0, currency = 'XAF', image_url, is_active = true } = req.body;
    const { rows } = await pool.query(
      `INSERT INTO service_catalogs (service_category, title, description, price, currency, image_url, is_active) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [service_category, title, description || null, price, currency, image_url || null, is_active]
    );
...
app.put('/api/catalogs/:id', adminAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    const { service_category, title, description, price, currency, image_url, is_active } = req.body;
    const { rows } = await pool.query(
      `UPDATE service_catalogs SET service_category=$1, title=$2, description=$3, price=$4, currency=$5, image_url=$6, is_active=$7 WHERE id=$8 RETURNING *`,
      [service_category, title, description || null, price || 0, currency || 'XAF', image_url || null, is_active !== false, id]
    );
    res.json({ success: true, item: rows[0] });
  } catch (err) {
...
app.delete('/api/catalogs/:id', adminAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
app.get('/api/site-settings', async (_, res) => {
  try {
    ...
    (rows as Array<{ key: string; value: string }>).forEach((r) => { obj[r.key] = r.value; });
    res.json(obj);
  } catch (err) {
...
app.put('/api/site-settings', adminAuth, async (req: AuthenticatedRequest, res) => {
  try {
...
app.get('/api/admin/stats', adminAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const stats = {
      ...
      jobs_per_company: (jobsPerCompanyR.rows || []).map((r: any) => ({ company_id: r.company_id, company: r.company, job_count: parseInt(r.job_count) })),
      applications_per_company: (appsPerCompanyR.rows || []).map((r: any) => ({ company_id: r.company_id, company_name: r.company_name, applications_count: parseInt(r.applications_count) })),
      recent_applications: (recentAppsR.rows || []).map((r: any) => ({ id: r.id, job_id: r.job_id, job_title: r.job_title, job_company: r.job_company, applicant_id: r.applicant_id, applicant_name: r.applicant_name, applicant_email: r.applicant_email, status: r.status, created_at: r.created_at })),
    };
  ...
  try {
    ...
    if (service) {
      result = await pool.query(
        `SELECT * FROM portfolios WHERE service_category = $1 ORDER BY featured DESC, created_at DESC`,
        [service]
      );
    } else {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO portfolios (title, description, image_url, project_url, service_category, featured)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [title, description || null, image_url || null, project_url || null, service_category, featured]
    );
...
app.put("/api/portfolios/:id", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE portfolios SET title=$1, description=$2, image_url=$3, project_url=$4, service_category=$5, featured=$6 WHERE id=$7 RETURNING *`,
      [title, description || null, image_url || null, project_url || null, service_category, featured || false, id]
    );
    res.json({ success: true, portfolio: rows[0] });
  } catch (err) {
    res.status(500).json({ success: false });
...
app.post('/api/user-documents', userAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.userId;
    const { doc_type, title, storage_url, metadata } = req.body;
    if (!doc_type) return res.status(400).json({ success: false, message: 'doc_type requis' });

    // Saved documents limit per document type (e.g. max 2 CVs, 2 letters)
    const { rows: savedTypeRows } = await pool.query(
      'SELECT COUNT(*) FROM user_documents WHERE user_id = $1 AND doc_type = $2',
      [userId, doc_type]
    );
    ...
    const { rows: monthRows } = await pool.query(
      `SELECT COUNT(*) FROM user_documents WHERE user_id=$1 AND doc_type=$2 AND created_at >= $3`,
      [userId, doc_type, startOfMonth]
    );
    ...
    const { rows } = await pool.query(
      `INSERT INTO user_documents (user_id, doc_type, title, storage_url, metadata) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
      [userId, doc_type, title || null, storage_url || null, metadata || null]
    );
...
app.get('/api/user-documents', userAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.userId;
...
app.delete('/api/user-documents/:id', userAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.userId;
    const { id } = req.params;
    const { rows } = await pool.query('SELECT user_id FROM user_documents WHERE id = $1', [id]);
    if (rows.length === 0) return res.status(404).json({ success: false, message: 'Document non trouvé' });
    if (rows[0].user_id !== userId) return res.status(403).json({ success: false, message: 'Accès refusé' });
    await pool.query('DELETE FROM user_documents WHERE id = $1', [id]);
    res.json({ success: true });
  } catch (err) {
...
app.get("/api/channels", async (_, res) => {
  try {
    const { rows } = await pool.query(
      "SELECT * FROM communication_channels WHERE is_active = true ORDER BY display_order ASC"
    );
    res.json(rows);
  } catch (err) {
    res.status(500).json({ success: false });
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO communication_channels (channel_name, channel_url, channel_type, icon_name, display_order, is_active)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [channel_name, channel_url, channel_type, icon_name || null, display_order, is_active]
    );
...
app.put("/api/channels/:id", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE communication_channels SET channel_name=$1, channel_url=$2, channel_type=$3, icon_name=$4, display_order=$5, is_active=$6 WHERE id=$7 RETURNING *`,
      [channel_name, channel_url, channel_type, icon_name || null, display_order || 0, is_active !== false, id]
    );
    res.json({ success: true, channel: rows[0] });
  } catch (err) {
    res.status(500).json({ success: false });
...
app.post('/api/interactions', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO visitor_interactions (user_id, service, event_type, payload, created_at) VALUES ($1,$2,$3,$4,NOW()) RETURNING *`,
      [user_id, service, event_type, payload]
    );
...
app.get('/api/admin/realtime', async (req, res) => {
  ...
  try {
    const decoded = jwt.verify(token, JWT_SECRET) as unknown as { id: number; role: string };
    const allowed = ['admin', 'super_admin', 'admin_content'];
    if (!decoded.role || !allowed.includes(decoded.role)) return res.status(403).json({ success: false, message: 'Accès admin requis' });
  } catch (err) {
    return res.status(401).json({ success: false, message: 'Token invalide' });
  }
  ...
  (async () => {
    try {
      ...
      rows.reverse().forEach((r: any) => { sendEvent(r); lastId = Math.max(lastId, r.id); });
    } catch (err) {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      if (rows.length > 0) {
        rows.forEach((r: any) => { sendEvent(r); lastId = Math.max(lastId, r.id); });
      }
    } catch (err) {
...
app.post("/api/portfolios", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      "INSERT INTO portfolios (title, description, image_url, project_url, service_category, featured) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
      [title, description || null, image_url || null, project_url || null, service_category, featured || false]
    );
...
app.patch("/api/portfolios/:id", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      "UPDATE portfolios SET title = COALESCE($1, title), description = COALESCE($2, description), image_url = COALESCE($3, image_url), project_url = COALESCE($4, project_url), service_category = COALESCE($5, service_category), featured = COALESCE($6, featured) WHERE id = $7 RETURNING *",
      [title, description, image_url, project_url, service_category, featured, id]
    );
...
app.post('/api/upload', userAuth, async (req: AuthenticatedRequest, res) => {
  try {
...
app.get('/api/business-cards', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    if (!userId) return res.status(401).json({ success: false, message: 'Non authentifié' });

    const result = await pool.query(
      'SELECT * FROM business_cards WHERE company_id = $1 ORDER BY created_at DESC',
      [userId]
    );
...
app.post('/api/business-cards', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    if (!userId) return res.status(401).json({ success: false, message: 'Non authentifié' });
    ...
    const result = await pool.query(
      `INSERT INTO business_cards (company_id, title, full_name, phone, email, website, address, card_color) 
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) 
       RETURNING *`,
      [userId, title, full_name, phone, email || null, website || null, address || null, card_color || 'blue']
    );
...
app.delete('/api/business-cards/:id', userAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
    ...
    const card = await pool.query(
      'SELECT * FROM business_cards WHERE id = $1 AND company_id = $2',
      [cardId, userId]
    );
...
const startServer = async () => {
  ...
  app.get('/api/saved-jobs', userAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.userId;
      if (!userId) return res.status(401).json({ success: false, message: 'Utilisateur non authentifié' });
  ...
  app.post('/api/saved-jobs', userAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.userId;
      const { job_id } = req.body;
      
      if (!userId) return res.status(401).json({ success: false, message: 'Utilisateur non authentifié' });
      if (!job_id) return res.status(400).json({ success: false, message: 'Job ID requis' });
  ...
  app.delete('/api/saved-jobs/:jobId', userAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.userId;
  ...
  app.get('/api/saved-jobs/check/:jobId', userAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.userId;
      const { jobId } = req.params;
...
app.use((err: any, req: any, res: any, next: any) => {
    ...
    try {
      if (req.method === 'GET') {
        ...
        if (/\/api\/users(\/me|\/\d+)?/.test(req.path)) {
          const body: any = { success: false, message: 'Erreur serveur (fallback)', data: {} };
  ...
  const body: any = { success: false, message: err?.message || 'Erreur serveur inattendue' };
...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    if (!email || !subject || !message) return res.status(400).json({ success: false, message: 'Tous les champs sont requis' });
